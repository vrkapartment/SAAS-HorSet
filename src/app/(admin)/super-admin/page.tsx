...
export default function SuperAdminPage() {
  ...
  // ค้นหาและคัดกรอง
  const [activeTab, setActiveTab] = useState<"workspaces" | "users" | "invites" | "settings" | "subscriptions">("workspaces")
  ...
  // แท็บ "แผนการใช้งาน (Subscriptions)"
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      // บันทึกการตั้งค่า SlipOK/PromptPay ของ HorSet เอง (สำหรับรับชำระค่า subscription)
    ...
    if (!isDemo) {
      try {
        ...
        if (settingsRes.success && settingsRes.data) {
          ...
          // Load HorSet's own SlipOK/PromptPay settings
        ...
        // Load ข้อมูลแท็บ "แผนการใช้งาน (Subscriptions)" — ไม่ให้ล้มทั้งหน้าถ้าโหลดส่วนนี้พลาด (เช่น ยังไม่ได้รัน migration)
        try {
          await loadSubscriptionsData()
        } catch (subErr) {
          console.error("Failed to load subscriptions data:", subErr)
        }
  ...
  // โหลดข้อมูลแท็บ "แผนการใช้งาน (Subscriptions)" — เรียกทั้งตอนโหลดหน้าแรกและตอนกดรีเฟรชในแท็บนี้
  ...
  // ฟังก์ชันปรับแผน/สถานะ subscription ของ workspace ด้วยมือ (Super Admin Override)
  const handleUpdateSubscription = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("ไม่สามารถปรับแผน/สถานะ subscription ได้: " + err.message)
    } finally {
  ...
  // ป้ายสถานะ subscription (สีตามสถานะ คล้าย badge สิทธิ์ในแท็บบัญชีผู้ใช้งาน)
  ...
  const getSubscriptionStatusBadgeClass = (status: SubscriptionStatus) => {
    switch (status) {
      case "active":
  ...
  const getPaymentStatusBadgeClass = (status: SaasPaymentRow["status"]) => {
    switch (status) {
      case "verified":
        return "bg-teal-500/20 text-teal-400 border border-teal-500/10"
      case "pending":
      ...
      <div className="space-y-8 pb-12">
        ...
        <div className="flex p-1 bg-slate-900/80 border border-slate-800 rounded-2xl w-full max-w-2xl shadow-lg relative z-10 overflow-x-auto">
          {[
            ...
            { id: "subscriptions", label: "แผนการใช้งาน", icon: CreditCard }
          ].map((tab) => {
        ...
        {activeTab === "settings" && (
          <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto">
            ...
            {/* กล่องตั้งค่า SlipOK/PromptPay ของ HorSet เอง (สำหรับรับชำระค่า subscription จากเจ้าของหอพัก) */}
            <div className="bg-slate-900/50 backdrop-blur-md rounded-3xl border border-slate-800 p-6 md:p-8 relative overflow-hidden group">
        ...
        {/* แท็บแผนการใช้งาน (Subscriptions) */}
