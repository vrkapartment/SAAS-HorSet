  ...
  const loadData = async () => {
    ...
    if (!isDemo) {
      try {
        ...
        // Load PDF Template ของแบบฟอร์ม ภ.ง.ด. 90/94
        const templatesRes = await getTaxFormTemplatesAction()
        if (templatesRes.success) {
          setTaxTemplates(templatesRes.data || [])
        }
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex items-center gap-3 shrink-0 self-start md:self-center">
              <button
                ...
                className="px-5 py-3 rounded-2xl bg-emerald-600/10 border border-emerald-500/20 hover:bg-emerald-600/20 text-emerald-400 hover:text-emerald-300 transition-all text-xs font-semibold flex items-center gap-2 shadow-lg"
              >
